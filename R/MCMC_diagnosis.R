...
#' generate trace‑plots for specified parameters, and optionally perform positivity
...
#'   (default: `c("beta0","gamma0","beta_Y","beta_A","gamma_Y","gamma_A")`).
#'   These are passed as regex to **bayesplot**'s `mcmc_trace()`.
#' @param save_plots Logical; if `TRUE`, saves each trace‑plot as a PNG file using
#'   `plot_prefix`.
...
#' @param pars Character vector of parameter patterns to plot.
...
#'
#' @author Originally by package authors; streamlined to align with new
#'   `_obs` / `_prev` naming convention.
#'
#' @examples
#' \dontrun{
...
mcmc_diagnosis <- function(fit_out,
  ...
  if (positivity) {
    if (is.null(fit_out$data_preprocessed)) {
      ...
      # exclude outcome & history variables; keep remaining covariates
      exclude <- c("pat_id", "k_idx", "Y_obs", "T_obs", "Y_prev", "T_prev", "A")
      covariates <- setdiff(names(df), exclude)

      formula_A <- stats::as.formula(paste("A ~", paste(c("Y_prev", "k_idx", covariates), collapse = " + ")))
