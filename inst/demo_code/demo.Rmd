...
We fit a joint Bayesian model for (i) the terminal process $T_k$ and (ii) the recurrent events $Y_k$, sharing the discrete time structure and history records. Each sub-model includes: the current treatment, lagged outcomes (e.g., $I(lagY_k^2)$), standardized covariates $(L.1, L.2)$, and a piecewise-constant baseline (time_baseline_T[k], time_baseline_Y[k]).

We expose variable names in diagnostics and summaries as follows:

-   $\beta_{T:L.1},\ \beta_{T:L.2},\ \beta_{Y:L.1},\ \beta_{Y:L.2}$: covariate ($L.1$, $L.2$ here) effects in $T$/$Y$ mode

-   $\theta_{T,\text{lag}:I(\text{lagYk}^2)},\ \theta_{Y,\text{lag}:I(\text{lagYk}^2)}$: coefficients for lagged outcome features.
...
=======
diag <- mcmc_diagnosis(fit,positivity     = FALSE)
...
Posterior draws $\{\Delta^{(m)}(s)\}_m$ are summarized by their mean and 95% credible interval.
